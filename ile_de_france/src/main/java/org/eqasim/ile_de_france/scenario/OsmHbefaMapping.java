...
public class OsmHbefaMapping extends HbefaRoadTypeMapping {
    ...
    private String getHEBFAtype(String roadType, double freeVelocity) {
        ...
        //BYIN 2025: mapping types to Hbefa factors (input files) correctly
        int capped_speed_m =(int) Math.floor((capped_speed) / 10) * 10;
        if (capped_speed_m >= 110) {
            if (hbfeaMap.get(type).name.equals("Trunk-Nat.")){
                return "RUR/" + "Trunk" + "/" + capped_speed_m;
            }
            if (hbfeaMap.get(type).name.equals("MW-Nat.")){
                return "RUR/" + "MW" + "/" + capped_speed_m;
            }
        }

        return "URB/" + hbfeaMap.get(type).name + "/" + capped_speed_m;
